...
#SBATCH --nodelist=node1,node2  # Replace with your node names
#SBATCH --tasks-per-node=1      # One task per node for Ray
#SBATCH --cpus-per-task=128     # Adjust based on your needs
#SBATCH --mem=1000GB            # Memory per node
#SBATCH --gres=gpu:8            # Number of GPUs per node
#SBATCH --exclusive
#SBATCH --output=./logs/mochi_%j.out
#SBATCH --error=./logs/mochi_%j.err
...
# Replace /path/to/train_XXXX.py with the actual path to your training script train_mochi.py or train_pusa.py
python -u /path/to/train_XXXX.py \
