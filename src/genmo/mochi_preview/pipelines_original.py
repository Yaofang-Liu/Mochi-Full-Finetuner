...
class MochiSingleGPUPipeline:
    ...
    def __call__(self, batch_cfg, prompt, negative_prompt, **kwargs):
        with torch.inference_mode():
            ...
            # Delete text_encoder to free up memory
...
def get_free_addr_and_port():
    """Find a free address and port to use for distributed training."""
    import socket
    # Get a list of available local IP addresses, starting from 127.0.0.2
    for ip in [f"127.0.0.{i}" for i in range(2, 255)]:
        try:
            with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
                s.bind((ip, 0))  # Bind to any available port
                s.listen(1)
                addr = ip
                port = s.getsockname()[1]
                return addr, port
            ...
            continue
    raise RuntimeError("Could not find a free local address")
...
class MultiGPUContext:
    def __init__(
        ...
        *,
        ...
        # Find a free port for this process group
        # if local_rank == 0:  # Only leader needs to find port
        #     port = find_free_port()
        # else:
        #     port = None
        # if world_size > 1:
        #     # Broadcast port from rank 0 to all other ranks
        #     port = torch.tensor([port if port is not None else 0], dtype=torch.long).cuda()
        #     dist.broadcast(port, src=0) 
        #     port = port.item()
        # # Find a free address and port for this process group
        # if local_rank == 0:  # Only leader needs to find address/port
        #     addr, port = get_free_addr_and_port()
        #     addr_port = torch.tensor([int(addr.split('.')[-1]), port], dtype=torch.long).cuda()
        # else:
        #     addr_port = torch.zeros(2, dtype=torch.long).cuda()
        
        # if world_size > 1:
        #     # Broadcast address and port from rank 0 to all other ranks
        #     dist.broadcast(addr_port, src=0)
        #     addr = f"127.0.0.{addr_port[0].item()}"
        #     port = addr_port[1].item()
