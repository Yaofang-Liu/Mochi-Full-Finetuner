...
class MochiModel(LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        B = batch_size 
        random_steps = torch.randint(0, num_total_steps - 1, (B,), device=z_0.device)
...
def train_mochi(config):
    ...
    if config['address'] is not None:
        ...
        ray.init(
            ...
            num_cpus=config['world_size']*12,  # 10 CPUs per worker
    ...
    scaling_config = ScalingConfig(
        ...
        resources_per_worker={
            "CPU": 8,  # Explicitly request 6 CPUs per worker
    ...
    try:
        ...
        # Clean up temporary files before shutdown
        try:
...
def parse_args():
    ...
    # parser.add_argument("--num_frames", type=int, default=163, help="Number of frames for video processing")
    parser.add_argument("--num_frames", type=int, default=83, help="Number of frames for video processing")
