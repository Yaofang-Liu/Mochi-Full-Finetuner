...
def gather_all_frames(x: torch.Tensor) -> torch.Tensor:
    ...
    # if cp_rank == 0:
        ...
        # """
        # print like:
        # (MultiGPUContext pid=3061095) gather_all_frames before: cp_rank: 0, x.shape: torch.Size([1, 128, 22, 240, 424]), max_T: 22
        # (MultiGPUContext pid=3061095) gather_all_frames after: cp_rank: 0, gathered_x num: 8, max_T: 22
        # (MultiGPUContext pid=3061095) gather_all_frames before: cp_rank: 0, x.shape: torch.Size([1, 256, 13, 120, 212]), max_T: 13
        # (MultiGPUContext pid=3061095) gather_all_frames after: cp_rank: 0, gathered_x num: 8, max_T: 13
        # gather_all_frames before: cp_rank: 0, x.shape: torch.Size([1, 12, 4, 60, 106]), max_T: 4
        # gather_all_frames after: cp_rank: 0, gathered_x num: 8, max_T: 4
        # """
...
class ContextParallelCausalConv3d(torch.nn.Conv3d):
    ...
    def forward(self, x: torch.Tensor):
        ...
        # print(f"ContextParallelCausalConv3d: cp_rank: {cp_rank}, cp_world_size: {cp_world_size}, x.shape: {x.shape}, context_size: {context_size},self.kernel_size: {self.kernel_size}, self.stride: {self.stride}") 
        """print:
